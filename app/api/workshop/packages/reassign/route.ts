...
export async function POST(request: Request) {
  ...
  try {
    ...
    const results = await runtimeEnv.DB.batch([
      ...
      runtimeEnv.DB.prepare("INSERT INTO package_assignment_history(id,package_id,from_order_id,to_order_id,reason,changed_by,changed_at) SELECT ?,?,?,?,?,?,? WHERE EXISTS(SELECT 1 FROM packages WHERE id=? AND order_id=?)").bind(crypto.randomUUID(), replacement.id, target.id, value.order_id, payload.reason, user.userId, now, replacement.id, value.order_id),
    ...
    if (!results[0].meta.changes || !results[1].meta.changes || !results[2].meta.changes || !results[3].meta.changes || !results[results.length - 1].meta.changes) return Response.json({ error: "package 상태가 변경되었습니다. 최신 내용을 다시 확인해주세요." }, { status: 409 });
