...
function workItemStatements(
  ...
  const dueAt = `${group.shipDate}T00:00:00+09:00`;
  return group.items.flatMap((item) => {
    ...
    return [
      ...
      `).bind(
        ...
        group.postalCode,
        group.roadAddr,
        group.roadAddrReference || null,
        group.jibunAddr || null,
        group.detailAddr,
        null,
        group.note,
        now,
...
export async function POST(request: Request) {
  ...
  const dates = [...new Set(pending.map((group) => group.shipDate))];
  ...
  for (const group of pending) {
    for (const item of group.items) {
      const product = productsByCode.get(item.productCode)!;
      const key = `${group.shipDate}\u0000${product.id}`;
      const current = requestedByProductDate.get(key) ?? { quantity: 0, groups: new Set<string>(), product, date: group.shipDate };
  ...
  if (capacityErrors.length) {
    return Response.json({ error: "발송일별 한정수량을 확인해주세요.", errors: capacityErrors }, { status: 409 });
  }
