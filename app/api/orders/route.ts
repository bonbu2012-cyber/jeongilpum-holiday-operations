...
async function serializeOrders(rows:OrderRow[]){
 ...
 return rows.map(row=>{
  ...
  return {id:row.id,orderNo:row.order_no,buyerName:row.buyer_name_snapshot,buyerPhone:row.buyer_phone_snapshot,status:row.order_status,fulfillmentType:row.fulfillment_type,scheduleLabel:row.schedule_label,fulfillmentId:fulfillment?.id??null,pickupAt:fulfillment?.pickup_at??null,shipDate:fulfillment?.ship_date??null,recipientName:fulfillment?.recipient_name??row.recipient_name,recipientPhone:fulfillment?.recipient_phone??row.recipient_phone,postalCode:fulfillment?.postal_code??null,roadAddress:fulfillment?.road_addr??row.road_address,roadAddrReference:fulfillment?.road_addr_reference??null,jibunAddr:fulfillment?.jibun_addr??null,detailAddress:fulfillment?.detail_addr??row.detail_address,customerArrived:Boolean(fulfillment?.customer_arrived),note:fulfillment?.note??row.customer_note,totalAmount:row.total_amount,version:row.version,submittedAt:row.submitted_at,items:itemResult.results.filter(item=>item.order_id===row.id).map(item=>({id:item.id,productId:item.product_id,name:item.product_name_snapshot,quantity:item.quantity,unitPrice:item.sale_unit_price})),packageCodes:packageResult.results.filter(item=>item.order_id===row.id).map(item=>item.package_code)};
 });
}
...
export async function GET(request:Request){
 ...
 try{
  ...
  if(q&&date){result=await runtimeEnv.DB.prepare(`SELECT o.* FROM orders o JOIN fulfillments f ON f.order_id=o.id WHERE (((f.fulfillment_type='pickup') AND substr(f.pickup_at,1,10)=?) OR ((f.fulfillment_type='shipping') AND f.ship_date=?)) AND (o.order_no LIKE ? OR o.buyer_name_snapshot LIKE ? OR o.buyer_phone_snapshot LIKE ? OR f.recipient_name LIKE ?) ORDER BY o.created_at DESC LIMIT 100`).bind(date,date,like,like,like,like).all<OrderRow>()}
  else if(date){result=await runtimeEnv.DB.prepare(`SELECT o.* FROM orders o JOIN fulfillments f ON f.order_id=o.id WHERE ((f.fulfillment_type='pickup') AND substr(f.pickup_at,1,10)=?) OR ((f.fulfillment_type='shipping') AND f.ship_date=?) ORDER BY o.created_at DESC LIMIT 100`).bind(date,date).all<OrderRow>()}
  else if(q){result=await runtimeEnv.DB.prepare(`SELECT * FROM orders WHERE order_no LIKE ? OR buyer_name_snapshot LIKE ? OR buyer_phone_snapshot LIKE ? OR recipient_name LIKE ? ORDER BY created_at DESC LIMIT 100`).bind(like,like,like,like).all<OrderRow>()}
  else{result=await runtimeEnv.DB.prepare("SELECT * FROM orders ORDER BY created_at DESC LIMIT 100").all<OrderRow>()}
