 ...
 return <div className="kiosk-app">
  ...
  <AnimatePresence mode="wait" custom={direction}>
   {step==="products"?<motion.div key="products" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} transition={transition} className="product-step">
    <aside className="category-rail">{categories.map(c=><button key={c} className={category===c?"active":""} onClick={()=>setCategory(c)}><span className={c==="뼈세트"?"category-symbol bone":"category-symbol"}>{c==="진공세트"?"VACUUM":c==="프리미엄"?"PREMIUM":c==="LA갈비"?"LA":c==="뼈세트"?"🦴":"O"}</span>{c}</button>)}<button className="inquiry" onClick={()=>alert("직원을 호출했습니다. 현재 선택 내용은 유지됩니다.")}><span>?</span>맞춤 주문<br/><small>직원 문의</small></button></aside>
...
function InfoStep({title,description,draft,setDraft,next,valid}:{title:string;description:string;draft:OrderDraft;setDraft:React.Dispatch<React.SetStateAction<OrderDraft>>;next:()=>void;valid:boolean}){
 ...
 return <div className="flow-card narrow"><div className="flow-heading"><small>고객정보</small><h1>{title}</h1><p>{description}</p></div><div className="form-stack"><Field label="주문자명 또는 회사명" value={draft.buyerName} onChange={v=>setDraft(d=>({...d,buyerName:v}))} placeholder="개인 이름 또는 회사명을 입력해주세요" error={attempted&&!nameValid?"주문자 이름 또는 회사명을 입력해주세요.":undefined}/><Field label="연락처" type="tel" value={draft.buyerPhone} onChange={v=>setDraft(d=>({...d,buyerPhone:v}))} placeholder="010-0000-0000" error={attempted&&!phoneValid?"휴대전화 번호를 숫자 10자리 이상 입력해주세요.":undefined}/><Field label="요청사항 (선택)" wide value={draft.note} onChange={v=>setDraft(d=>({...d,note:v}))} placeholder="예: 지방을 적게 부탁드립니다"/></div><div className={valid?"form-status ready":"form-status"}><span>{nameValid?"✓":"○"} 주문자명 또는 회사명</span><span>{phoneValid?"✓":"○"} 연락처 10자리 이상</span></div><button className="main-cta" onClick={handleNext}>다음 <span>→</span></button></div>
}
