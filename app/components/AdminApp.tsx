...
function OrderWorkspace({
  ...
  return (
    <section className="order-workspace">
      ...
      <div className="admin-order-list">
        {scheduled.length ? scheduled.map((order) => <OrderCard key={order.id} order={order} update={update} assignSchedule={assignSchedule} />) : <div className="empty-orders">{view === "search" ? "검색된 주문이 없습니다." : "해당 날짜에 주문이 없습니다."}</div>}
      </div>
      {view !== "search" && (
        <section className="unscheduled-orders">
          ...
          <div className="admin-order-list">
            {unscheduled.length ? unscheduled.map((order) => <OrderCard key={order.id} order={order} update={update} assignSchedule={assignSchedule} />) : <div className="empty-orders">일정 미지정 주문이 없습니다.</div>}
          </div>
        </section>
      )}
    </section>
