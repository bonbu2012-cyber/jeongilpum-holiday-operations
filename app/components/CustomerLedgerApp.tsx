...
function PaymentFields({ method, setMethod, amount, setAmount, paidAt, setPaidAt, payerName, setPayerName, payerPhone, setPayerPhone, payerRelation, setPayerRelation, memo, setMemo }: {
  ...
  return <div className="ledger-form-grid"><label><span>결제수단</span><select value={method} onChange={(event) => setMethod(event.target.value as PaymentMethod)}><option value="card">카드</option><option value="cash">현금</option><option value="bank_transfer">계좌이체</option></select></label><MoneyInput label="결제금액" min="1" value={amount} onValueChange={setAmount} /><label><span>결제일시</span><input type="datetime-local" value={paidAt} onChange={(event) => setPaidAt(event.target.value)} /></label><label><span>실제 결제자 이름 · 선택</span><input value={payerName} onChange={(event) => setPayerName(event.target.value)} /></label><label><span>실제 결제자 전화번호 · 선택</span><input value={payerPhone} onChange={(event) => setPayerPhone(event.target.value)} /></label><label><span>고객과의 관계 · 선택</span><input value={payerRelation} onChange={(event) => setPayerRelation(event.target.value)} /></label><label className="wide"><span>메모 · 선택</span><input value={memo} onChange={(event) => setMemo(event.target.value)} /></label></div>;
}
...
function ApplyConsultationForm({ customerId, consultation, onDone, onCancel }: { customerId: string; consultation: Consultation; onDone: () => void; onCancel: () => void }) {
  ...
  return <section className="ledger-editor"><h3>상담 내용 장부에 적용</h3><p>{consultation.note}</p><label><span>새 장부 표시명 · 선택</span><input value={ledgerLabel} onChange={(event) => setLedgerLabel(event.target.value)} placeholder="예: 가족 대리결제 분리" /></label><MoneyInput label="새 장부로 이관할 입금액" min="0" value={transferAmount} onValueChange={setTransferAmount} /><label><span>상담 후 적용 내용</span><textarea value={applicationMemo} onChange={(event) => setApplicationMemo(event.target.value)} /></label><label><span>관리자 패스워드 · 적용 시 재확인</span><input type="password" value={adminPassword} onChange={(event) => setAdminPassword(event.target.value)} /></label>{error && <p className="payment-error" role="alert">{error}</p>}<div><button onClick={onCancel}>취소</button><button className="task-primary" disabled={saving || !applicationMemo.trim() || !adminPassword} onClick={() => void save()}>{saving ? "적용 중…" : "장부 분리 적용"}</button></div></section>;
}
