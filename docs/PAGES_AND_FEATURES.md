...
- 고정 계절 문구를 사용하지 않는다.
- 직원 도움 버튼은 어느 화면에도 표시하지 않는다.
- 금액 직접입력·수정란은 천 단위 구분과 한글 금액 읽기를 함께 제공한다.
...
- 품명, 금액, 요청사항(선택)을 직접입력
- 금액은 1원 이상의 정수로 validation하고 입력 중 천 단위 구분과 한글 금액을 표시
- 완료 후 main kiosk draft의 `customItem`에 병합
- `/kiosk?resume=cart`로 복귀
- 최종 주문에서 기존 customization row 구조에 품명과 요청사항을 호환 저장
- 판매장과 작업장은 맞춤 품명·금액을 표시하고 요청사항 버튼으로 상세를 확인
