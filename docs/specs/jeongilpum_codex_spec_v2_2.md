...
# 51. 판매장 메인 UI v3 — 엑셀형 운영판

판매장 메인은 카드형 대시보드가 아니라 **날짜별 주문을 한눈에 보는 고밀도 표형 운영판**으로 구현한다.
...
- 기본은 미완료 우선
- 완료건은 하단 접기/필터로 확인
- 전화번호, 결제금액, 전체 주소는 표에 표시하지 않음
- 행 전체 클릭 가능
- 행 클릭 시 주문상세 drawer/modal 또는 상세페이지
- 인라인 수량 수정 금지
- 주문 변경은 반드시 주문 상세 안에서 수행

예:
10:00 | 김철수 | 봉황2·진1 | 3 | 방문 | 준비중 | -
10:30 | 박영희 | 팔영2 | 2 | 방문 | 준비완료 | 변경
발송 | 김민수 | LA1호×2 | 2 | 택배 | 접수 | -
...
DB 내부 상태와 UI 문구는 domain mapper로 분리한다.
...
주문 변경은 메인 표에서 직접 할 수 없다.

# 52. 여분 완성품 / 쇼케이스 현황

판매장 메인 하단 또는 별도 subpanel에 여분 완성품 표를 표시한다.

예:
봉황 | 작업장 2 | 쇼케이스 1 | 총 가용 3
팔영 | 작업장 1 | 쇼케이스 0 | 총 가용 1
LA갈비 1호 | 작업장 3 | 쇼케이스 2 | 총 가용 5
...
주의:
- 예약고객에게 이미 배정된 package는 `여분 완성품`에 포함하지 않는다.
- 판매장 여분 수량은 `package_status=available`인 package만 집계한다.

## 52.2 쇼케이스 판매 P0

쇼케이스 상품이 실제 판매되면 반드시 웹앱에서 `판매 처리`가 필요하다.
...
3. 결제 기록
4. 판매 완료
5. display_sale order 생성
6. package assignment 생성
7. package_status=handed_over 또는 sold-equivalent
8. location 변경
9. 판매량/매출/작업량/출고량 통계 반영
10. 쇼케이스 가용수량 즉시 감소

웹앱에 기록되지 않은 쇼케이스 판매는 통계 누락이므로 P0 운영 오류로 취급한다.

# 53. 고객 키오스크 → 판매장 실시간 연동 P0

현재 MVP에서 가장 먼저 수정해야 할 핵심 요구사항.

## 53.1 목표

고객이 `/kiosk`에서 주문 접수 성공:
1. 서버가 Supabase에 주문을 transaction으로 저장
2. `orders`, `order_items`, `fulfillments`, `fulfillment_items` 저장 완료
3. order_status=`submitted`
4. 판매장 `/admin`의 현재 날짜 주문표에 **즉시 나타남**
5. 판매장 새로고침 없이 반영하는 것을 기본 목표로 함
6. 실시간 구독이 일시 실패해도 focus/refetch/polling fallback으로 최종 일관성 확보

`주문 완료` 화면은 판매장용 데이터 저장 transaction이 commit된 뒤에만 보여준다.

## 53.2 절대 금지 구조
- kiosk localStorage에만 주문 저장
- kiosk mock array에만 주문 추가
- admin이 별도 mock 데이터 사용
- kiosk와 admin이 서로 다른 API/data source 사용
- 서버 저장 실패인데 kiosk 완료 화면 표시
- realtime event만 믿고 DB query를 하지 않는 구조

**Single Source of Truth = Supabase Postgres**

# 54. 주문 생성 서버 흐름

권장:
`POST /api/kiosk/orders`
또는 Next.js Server Action.

처리:
1. payload schema validation
2. idempotency_key 확인
3. 서버에서 현재 product 가격/판매상태 재검증
4. transaction/RPC 실행
5. order 생성
6. order_items 생성
7. fulfillment 생성
8. allocation 검증
9. order_event `order_submitted`
10. commit
11. 생성된 order id/order_no 반환

서비스 역할키는 서버에서만 사용.

가능하면 Postgres RPC:
`create_order_transaction(payload jsonb)`

transaction 실패 시 모든 row rollback.

# 55. 판매장 주문 query

판매장 날짜별 표는 반드시 DB query로 생성.

예시 논리:
- 선택 날짜의 pickup fulfillment
- 선택 날짜의 shipping ship_date
- 필요한 gift fulfillment
- cancelled 제외
- order + items + fulfillment의 필요한 필드 join

클라이언트에서 mock array 사용 금지.

query key 예:
`['sales-orders', selectedDate, filters]`

TanStack Query 사용 가능하나 필수는 아님.
사용할 경우 realtime event 발생 시 invalidate/refetch.

# 56. Supabase Realtime 연동

판매장 화면 mount 후 관련 테이블을 구독.
...
권장 흐름:
1. DB change event 수신
2. 현재 선택 날짜와 관련 가능성이 있으면
3. 판매장 주문 query invalidate
4. DB를 다시 읽어 최신 표 구성

Realtime payload 자체를 복잡한 최종 UI 데이터로 믿지 않는다.

# 57. Realtime 연결 장애 fallback

판매장:
- window focus 시 refetch
- network reconnect 시 refetch
- 선택 날짜 변경 시 refetch
- 수동 `새로고침` 버튼
- 필요 시 15~30초 low-frequency polling fallback

UI:
- 정상: `실시간 연결됨`
- 문제: `연결 확인 중`
- 장시간 실패: `실시간 연결이 불안정합니다. 새로고침`

연결장애가 있어도 DB 저장 주문이 유실되어서는 안 된다.

# 58. Realtime 구독 권한 / RLS

sales/admin authenticated user만 필요한 주문 데이터를 구독 가능해야 한다.
anon kiosk는 판매장 주문 feed 구독 불가.
Supabase Realtime 사용 시 해당 테이블 publication/realtime 설정 및 RLS가 실제로 동작하는지 테스트한다.

# 59. 신규 주문 시 판매장 UX

키오스크에서 신규 주문이 등록되면 판매장:
- 선택 날짜가 해당 주문 날짜라면 표에 자동 등장
- 필요하면 짧은 `신규 주문` 강조 2~3초
- 과도한 popup 금지
- 주문변경과 신규주문은 알림 강도를 구분

주문이 100건 이상일 때 새 주문 때문에 사용자의 스크롤 위치를 강제로 이동시키지 않는다.
상단에 `새 주문 1건` 버튼을 보여주고 사용자가 원할 때 최신 위치로 이동 가능.

# 60. 주문변경 → 판매장/작업장 동기화

주문 상세에서 변경 확정:
1. optimistic concurrency/version 검증
2. transaction으로 주문 변경
3. order_event 생성
4. 영향 package/label 계산
5. operational_alert 생성
6. commit
7. 판매장 query refetch
8. 작업장 realtime alert
9. 해당 판매장 row에 `변경` 표시
10. 작업장 acknowledgement 후 표시 업데이트

# 61. 키오스크-판매장 연동 Acceptance Criteria

반드시 검증:
1. 판매장 `/admin`을 브라우저 A에서 열어둔다.
2. `/kiosk`를 브라우저 B에서 연다.
3. 오늘 방문수령 주문을 1건 접수한다.
4. kiosk 완료화면이 나타난다.
5. 2~3초 이내 판매장 오늘 표에 새 주문이 표시된다.
6. 판매장 새로고침 없이 표시되어야 한다.
7. 판매장 행 클릭 시 kiosk에서 입력한 상품/수량/고객/수령정보와 일치한다.
8. 주문을 연속 두 번 제출해도 1건만 표시된다.
9. 판매장 realtime 연결을 끊었다가 복구하면 누락 주문이 refetch로 나타난다.
10. 판매장 새로고침 후에도 동일 주문이 존재한다.
11. kiosk localStorage를 삭제해도 서버 주문은 유지된다.
12. admin mock 데이터 없이 실제 Supabase row만 표시한다.

이 Acceptance Criteria를 통과하지 못하면 Phase 1 완료로 간주하지 않는다.

# 62. Codex 즉시 수정 지시

현재 구현에서 고객 주문이 판매장에 연동되지 않는 문제를 P0 버그로 처리한다.

먼저 다음을 조사:
1. kiosk 주문이 실제 Supabase `orders`에 INSERT되는지
2. `order_items`, `fulfillments`가 함께 생성되는지
3. kiosk가 mock/local state만 사용하는지
4. admin이 Supabase query를 사용하는지
5. kiosk/admin이 같은 Supabase project/environment를 사용하는지
6. RLS 때문에 admin SELECT가 막히는지
7. Realtime publication/subscription이 설정됐는지
8. admin selectedDate query가 신규 주문 날짜를 올바르게 포함하는지

수정 후:
- kiosk와 admin이 동일 Supabase DB를 source of truth로 사용
- transaction order create
- admin date table DB query
