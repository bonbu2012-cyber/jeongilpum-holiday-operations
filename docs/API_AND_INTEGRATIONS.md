...
- 일반 상품은 client가 보낸 가격을 받지 않고, 맞춤주문만 직원이 합의한 수동 금액을 받는다.
- 상품 ID와 수량으로 D1 상품을 조회해 총액을 계산한다.
- 방문수령은 pickup date/time, 택배는 ship date와 주소를 검증한다.
- custom item은 직접 입력한 품명과 1원 이상의 정수 금액을 검증하고 요청사항을 작업 구성 정보로 저장한다.
- 기존 session 초안 호환을 위해 이전 예산 필드도 수용하며, 새 주문은 품명·금액 필드를 사용한다.
