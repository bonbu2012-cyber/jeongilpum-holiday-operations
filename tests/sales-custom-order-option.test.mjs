...
test("WorkItemEditor includes '기타/맞춤주문' (custom-order) in productOptions and handles price reset", async () => {
  ...
  assert.match(source, /draft\.productId === "custom-order"\s*\?\s*"구성 정보 \(기타\/맞춤주문/);
});

test("orders API prepareManualWorkItems sets productName to '기타/맞춤주문' for custom-order", async () => {
  const source = await read("app/api/orders/route.ts");

  assert.match(source, /item\.productId === "custom-order"\s*\?\s*\(item\.productName \|\| "기타\/맞춤주문"\)\s*:\s*product\.name/);
  assert.match(source, /productName:\s*clean\(item\.productName\)/);
});

test("work-items API PATCH and POST set productName to '기타/맞춤주문' for custom-order", async () => {
